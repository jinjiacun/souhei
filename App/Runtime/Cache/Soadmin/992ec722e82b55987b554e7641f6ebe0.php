		  ...
		  <tr>
			...
			<td width="550">查看内容：
			 状&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;态：<select name="status" style="width: 180px">
                         ...
                         &nbsp;&nbsp;会员id：<input type="text" name="user_id" value="<?php echo ($user_id); ?>"/>
                         &nbsp;&nbsp;类型:<select name="type" style="width:180px;">
                         <option value=0 >---请选择类型---</option>
                         <option value='005001' <?php if('005001' == $type): ?>selected="selected"<?php endif; ?>>点赞</option>
                         <option value='005002' <?php if('005002' == $type): ?>selected="selected"<?php endif; ?>>提问</option>
                         <option value='005003' <?php if('005003' == $type): ?>selected="selected"<?php endif; ?>>加黑</option>
                         </select>
          ...
          <td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
		    ...
		    <td width="6%" align="center" bgcolor="#EEEEEE">选择</td>
                  ...
                  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr align="center">
		                <td bgcolor="#FFFFFF">
                      <input type="checkbox" name="id[]" value="<?php echo ($item["id"]); ?>|<?php echo ($item["company_id"]); ?>"/>
                      <?php echo ($item["id"]); ?></td>
                    ...
                    <?php if('006001' == $auth_level_company[$item['company_id']]): ?><a href="/yms/index.php/Home/Query/query_hpt/resid/<?php echo ($item["company_id"]); ?>" target="_black"><?php echo ($company_list[$item['company_id']]); ?>
                    </a>
                    <?php elseif('006002' == $auth_level_company[$item['company_id']]): ?>
                    <a href="/yms/index.php/Home/Query/query_wrz/resid/<?php echo ($item["company_id"]); ?>" target="_black"><?php echo ($company_list[$item['company_id']]); ?>
                    </a>
                    <?php elseif('006003' == $auth_level_company[$item['company_id']]): ?>
                    <a href="/yms/index.php/Home/Query/query_yrz/resid/<?php echo ($item["company_id"]); ?>" target="_black"><?php echo ($company_list[$item['company_id']]); ?>
                    </a><?php endif; ?>
                    </td>
                    <td height="20" bgcolor="#FFFFFF" style="word-break: break-all">
                    <?php if('005001' == $item['type']): ?><font style="color:#d53315">点赞</font>
                    <?php elseif('005002' == $item['type']): ?>
                    <font style="color:#0086d2">提问</font>
                    <?php else: ?>
                    <font style="color:#008000">加黑</font><?php endif; ?>
                    </td>                    
                    <td height="20" bgcolor="#FFFFFF" style="word-break: break-all">
                    ...
                    <?php else: ?>
                    <?php echo (date("Y-m-d H:i:s",$item["validate_time"])); endif; ?>
                    </td>
                    <td bgcolor="#FFFFFF">
                        <?php if(0 == $item['is_validate'] and 0 == $item['is_delete']): ?><a href="javascript:request_get('/yms/index.php/Soadmin/Comment/validate/id/<?php echo ($item["id"]); ?>/parent_id/<?php echo ($item["parent_id"]); ?>/company_id/<?php echo ($item["company_id"]); ?>');">审核</a><?php endif; ?>
                        <?php if(0 == $item['is_delete']): ?><a href="javascript:request_get('/yms/index.php/Soadmin/Comment/delete/parentid/<?php echo ($item["parent_id"]); ?>/id/<?php echo ($item["id"]); ?>/company_id/<?php echo ($item["company_id"]); ?>/is_validate/<?php echo ($item["is_validate"]); ?>');" onclick="return confirmdel('确定要删除吗')">删除</a><?php endif; ?>
                     </td>
